...
class TESAnalyze():
    ...
    def ivInterpolate(self, Vtes, Ites, Rtes, percentRns, rn, tran_perRn_start=0.050, plot=False):
        ...
        if plot: 
            plt.figure()
            plt.plot(v_pnts*1e6, i_pnts*1e3, 'o', alpha=0.8)
            plt.xlabel('V [mV]'); plt.ylabel('I [uA]')
...
def fit_powerlaws(ivs, save_figs=False, fitGexplicit=True, perRn_toquote=80, Tb_inds=[0, 3, 8, 11, 15, 2], fn_comments='', Tbq_ind=2, constT=True, 
                    perRn=np.array([25, 30, 40, 50, 60, 70, 80]), init_guess=np.array([1.E-10, 2.5, .170]), show_psatcalc=False, bolotest_dir='/Users/angi/NIS/Bolotest_Analysis/'):
    ...
    for tesid in tesids:
        ...
        GTcs, Ks, ns, Ttes, GTcs_err, Ks_err, ns_err, Ttes_err = tes.fitPowerLaw(perRn, Tbaths, p_pnts.T, init_guess, fitToLast=True, 
                suptitle=tesid, plot=True, sigma=sigma_p.T, pfigpath=pfig_path, constT=constT, fitGexplicit=fitGexplicit)   # pass error to fitter     
        ...
        if show_psatcalc:   # double-check Psat calculation (i've never seen this not work)
            ...
            if save_figs: plt.savefig(bolotest_dir + 'Plots/past_calc/' + tesid + '_psatcalc' + fn_comments + '.png', dpi=300) 
...
def savedata(ivs, pkl_file, csv_file):
    ...
    fields = np.array(['TES ID', 'Bolometer', 'Pad', 'Tc [mK]', 'Tc_err [mK]', 'Rn [mOhms]', 'Rn_err [mOhms]', 'G@Tc [pW/K]', 'G_err@Tc [pW/K]', 'G@170mK [pW/K]', 
                        'G_err@170mK [pW/K]', 'k', 'k_err', 'n', 'n_err', 'Psat@'+str(round(Tb_toquote*1e3))+'mK [pW], IV', 'Psat_err@'+str(round(Tb_toquote*1e3))+'mK [pW], IV', 'Psat@'+str(round(Tb_toquote*1e3))+'mK [pW], Calc'])  
    # rows = np.array([[ivs[tesids[pp]][field] for field in fields] for pp in np.argsort(pads)])
    rows = np.array([[tesids[pind], boloids[pind], pads[pind], Tcs[pind], Tcerrs[pind], Rns[pind], Rnerrs[pind], GTcs[pind], GTcerrs[pind], G170s[pind], G170errs[pind], 
                    ks[pind], kerrs[pind], ns[pind], nerrs[pind], PsatIVs[pind], Psaterrs[pind], PsatCalcs[pind]] for pind in np.argsort(pads)])
    # pdb.set_trace()
...
def plotfit_singlebolo(ivs, tesid, save_figs=False, fn_comments='',
                    perRn=np.array([25, 30, 40, 50, 60, 70, 80]), init_guess=np.array([1.E-10, 2.5, .170]), show_psatcalc=False, bolotest_dir='/Users/angi/NIS/Bolotest_Analysis/'):
